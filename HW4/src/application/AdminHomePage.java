...
public class AdminHomePage {
    ...
    public void show(Stage primaryStage, User user) {
        ...
        switchRole.setOnAction(e -> {
            ...
            System.out.println("🔁 Switch Role clicked. Selected role: " + selectedRole);

            if (selectedRole != null) {
                switch (selectedRole.toLowerCase()) {
                    case "student":
                        System.out.println("➡️ Loading StudentHomePage");
                        new StudentHomePage(databaseHelper).show(primaryStage, user);
                        break;
                    case "staff":
                        System.out.println("➡️ Loading StaffHomePage");
                        new StaffHomePage(databaseHelper).show(primaryStage, user);
                        break;
                    case "instructor":
                        System.out.println("➡️ Loading InstructorHomePage");
                        new InstructorHomePage(databaseHelper).show(primaryStage, user);
                        break;
                    case "admin":
                        System.out.println("➡️ Loading AdminHomePage");
                        new AdminHomePage(databaseHelper).show(primaryStage, user);
                        break;
                    case "reviewer":
                        System.out.println("➡️ Loading ReviewerHomePage");
                        new ReviewerHomePage(databaseHelper).show(primaryStage, user);
                        break;
                    case "user":
                        System.out.println("➡️ Loading UserHomePage");
                        new UserHomePage(databaseHelper).show(primaryStage);
                        break;
                    default:
                        System.out.println("⚠️ Unknown role selected: " + selectedRole);
                }
            }
        });
        ...
        layout.getChildren().addAll(
            ...
            listUsersButton, assignRoleButton, inviteButton, deleteButton, roleDropdown, switchRole, logoutButton
        );
